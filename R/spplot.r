    ...
    for(p in 2:length(data))  
    {
        for(i in 2:length(data[[p]][["IMUT"]]))
        {
            ...
            for(l in lines)
            {
                ...
                index_relcoord_map[[ arr[1] ]][[ "b" ]]=as.numeric(arr[14])
                index_relcoord_map[[ arr[1] ]][[ "y" ]]=
            ...
            if(file.exists(paste(tandir,"/",fr,sep="")))
            {
                ...
                for(i in 1:length(m.label))
                {
                    fragment_coord<-as.numeric(gsub("(\\d+)","\\1",
                        regmatches(m.label[i],gregexpr("(\\d+)",m.label[i]))));	
                    if(grepl(pattern="y",x=m.label[i]) == TRUE)
                    {
                        mz_y[i]=m.mz[i]
                        label_y[i]=m.label[i]
                        int_y[i]=m.int[i]
                        if(fragment_coord>=index_relcoord_map[[bn]][["y"]])
                        {
                            colors_y[i]="brown3";
                        }
                        else
                    ...
                    else
                    {
                        ...
                        label_b[i]=m.label[i]
                        int_b[i]=m.int[i]
                        if(fragment_coord>=index_relcoord_map[[bn]][["b"]])
                        {
                            colors_b[i]="brown3";
                        }
                        else
                ...
                if(length(mz_b)>0)
                {
                    ...
                    text(mz_b,-15,labels=label_b,col=colors_b,xpd=TRUE,srt=90)
                ...
                if(length(mz_y)>0)
                {
                    ...
                    text(mz_y,145,labels=label_y,col=colors_y,xpd=TRUE,srt=90)
