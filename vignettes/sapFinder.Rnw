...
you also can obtain them,which I've downloaded recently,from the 
\verb|sapfinder_pipeline| repository in bitbucket 
(\url{https://bitbucket.org/xushaohang/sapfinder_pipeline}).You can find 
them in the \verb|"annotation_files"| directory.
